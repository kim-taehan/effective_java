...
- Item84: 프로그램 동작을 스레드 스케줄러에 기대지 마라.
```text
...
## 12장 직렬화
> 객체 직렬화란 자바가 객체를 바이트 스트림으로 인코딩하는 것이다.
> 이 부분은 따로 정리를 해야 겠다

- Item85: 자바 직렬화의 대안을 찾으라.
```text
- 새로운 시스템에서 자바 직렬화를 써야 할 이유는 없다. 
- JSON 과 같은 데이터 표현을 사용하자.
```

- Item86: Serializable을 구현할지는 신중하게 결정하라.
```text
- 클래스의 인스턴스를 직렬화 할 수 있게 하려면 implements Serializable 만 선언하면 된다. 
- Serializable 구현의 보안에 구멍이 생길 위험이 높기 때문에 가볍게 결정할 사안이 아니다.
```

- Item87: 커스텀 직렬화 형태를 고려해보라.
```text
- writeObject(직렬화), readObject(역질렬화) 를 구현해 커스텀 직렬화가 가능하다.
- transient 한정자: 해당 객체의 논리적 상태와 무관한 필드라고 선언 (직렬화 대상에서 제외되며, 역직렬화시 null, 0으로 대체)
- serialVersionUID: 속도 향상기능 이며, 잠재적인 호환성 문제가 사라진다. 
```

- Item88: readObject 메서드는 방어적으로 작성하라.
```text
- private 인 객체 참조 필드는 객체를 방어적으로 복사해야 한다. 
- 불변식을 검사하고 어긋나는게 발견되면 InvalidObjectException을 던져라
- 재정의 할 수 있는 메서드는 호출하지 마라.
```

- Item89: 인스턴스 수를 통제해야 한다면 readResolve보다는 열거 타입을 사용하라.
```text
- readResolve 를 통해 싱글톤 객체를 다수를 생성할 수 있다.
```
